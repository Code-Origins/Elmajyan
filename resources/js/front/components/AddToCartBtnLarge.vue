<template>
    <div>
        ...
        <div class="d-flex mt-2 add-to-cart-box" :class="{ outOfStock: propProduct.stock_status === 'out_of_stock' }">
            <a href="javascript:" @click="handleAddToCart" :class="{ 'added': isAddedToCart(propProduct.id), 'disabled': propProduct.stock_status === 'out_of_stock' }">
                <span v-if="propProduct.stock_status !== 'out_of_stock' && isAddedToCart(propProduct.id)">Ավցելացված է</span>
                <span v-if="propProduct.stock_status !== 'out_of_stock' && !isAddedToCart(propProduct.id)">Ավելացնել զամբյուղի մեջ</span>
                <span v-if="propProduct.stock_status === 'out_of_stock'">Ավելացնել զամբյուղի մեջ</span>
            </a>
        </div>
    </div>
</template>
