...
////////add padding
    ...
    if(url.includes('/product-category/')) {
        const segment = decodeURI(document.location.href.split('/product-category/').reverse()[0].split('/').reverse()[0].split('?')[0]);
        $(`body .category-list li.cat-item.cat_slug_${segment}`).addClass('active').parents('li.dropdown').addClass('active').parents('li.dropdown').addClass('active')
    }
